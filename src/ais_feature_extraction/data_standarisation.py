...
for i in range(len(final_df)):
    ...
    # utm_module.ED(N0, E0, N1, E1)).round(1)
    final_df.loc[i, 'OS_distance_TS'] = format(utm_module.meter_to_NM(
        utm_module.ED(N0, E0, N1, E1)).round(1), '.6f')

    final_df.loc['OS_head_inter_TS'] = utm_module.head_inter(
        heading0, heading1, to_2pi=True)
...
# normalise the data
# final_df[2.999998 <= final_df.OS_distance_TS <= 3.000001] = 3.0
...
# final_df['OS_distance_goal'] = utm_module.meter_to_NM(final_df['OS_distance_goal'])
